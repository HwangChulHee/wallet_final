  ...
  const confirmMnemonic = async () => {
    ...
    return (
      <>
        ...
        <div className='card-container'>
          <div className='h-w-p-90'>
              <h2 className='h-text-center h-my-4'>비밀 복구 구문 확인</h2>
          ...
          <div className='card-content'>
              <div className="h-w-full h-flex h-flex-row h-items-center h-justify-center">
                <CircleText2 words={recoveryWords} handleWordChange={handleWordChange} />
              </div>
              <div className='h-w-full h-mb-10'>
                {isSomeInput ? (
                    <div className='h-p-3 h-text-left h-text-red-500 h-text-xs '>
                    12개의 비밀 복구 구문을 모두 입력해주세요.
                    </div>
                ) : null}
              ...
              <div className='btn-group h-mb-4'>
                  <button onClick={handleInitMnemonic} disabled={!isCorrect} className='h-mb-2'>다음</button>
              </div>
          </div>                
        </div>
      </>
